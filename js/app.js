...
const LIVE_TRANSCRIPT_MAX_VISIBLE_ROWS = 160;
const LIVE_TRANSCRIPT_BUILD_SEGMENT_LIMIT = LIVE_TRANSCRIPT_MAX_VISIBLE_ROWS * 4;
...
const state = {
  ...
  manualSpeakerUiLastRenderedAt: 0,
  ...
  transcriptVisibleRowCount: 0,
  transcriptHiddenRowCount: 0,
  transcriptRenderFrame: 0,
};
...
function updateSessionDurationClock() {
  if (!elements.durationValue || !elements.speechOnlyValue) return;
  if (!state.currentSession) {
...
function updateManualSpeakerTimerClock() {
...
function shouldUpdatePlaybackHighlightOnClock() {
...
function tickClockUi() {
  updateSessionDurationClock();
  const shouldRefreshManualSpeakerUi = Boolean(
    state.currentSession && (state.currentSession.status === 'active' || state.manualSpeakerEvents.length || state.manualSpeakerEditingEventId)
  );
  if (shouldRefreshManualSpeakerUi && Date.now() - Number(state.manualSpeakerUiLastRenderedAt || 0) >= 1000) {
    renderManualSpeakerControls();
  } else {
    updateManualSpeakerTimerClock();
  }
  if (shouldUpdatePlaybackHighlightOnClock()) {
...
async function applyManualSpeakerSelection(nextRawLabel = getResolvedManualSpeakerSelection()) {
  ...
  if (state.currentSession) {
    syncManualSpeakerStateToSession();
    if (resolvedLabel !== previousLabel) {
      await persistCurrentSessionNow();
    }
  }
...
function createSpeakerFinalAssignmentState(session = state.currentSession) {
  const rawByManualLabel = new Map();
  const usedRawLabels = new Set();

  buildManualLabelToCanonicalRawMap(session).forEach((rawSpeaker, normalizedLabel) => {
    const normalizedRawSpeaker = String(rawSpeaker || '').trim();
    if (!normalizedRawSpeaker) return;
    usedRawLabels.add(normalizedRawSpeaker);
    if (normalizedLabel) {
      rawByManualLabel.set(normalizedLabel, normalizedRawSpeaker);
    }
  });
...
function buildCanonicalFinalDiarizedSegments({ diarizedSegments = [], session = state.currentSession, assignmentState = null } = {}) {
  ...
  const seededSpeakerCount = parseSpeakerNames(session?.speakerNames || state.settings.speakerNames || '').length;
  ...
  diarizedSegments.forEach((segment) => {
    ...
    if (!localSpeakerAssignments.has(localSpeaker)) {
      if (bestManualMatch && bestManualMatch[1] >= 1200) {
        const [matchedLabel] = bestManualMatch;
        const label = normalizeManualSpeakerName(matchedLabel) || String(matchedLabel || '').trim() || 'Speaker';
        const normalizedLabel = normalizeTranscriptSpeakerKey(label);
        let rawSpeaker = rawByManualLabel.get(normalizedLabel);
        if (!rawSpeaker) {
          rawSpeaker = getCanonicalRawSpeakerLabelForManualLabel(label, session, usedRawLabels);
          rawByManualLabel.set(normalizedLabel, rawSpeaker);
        } else {
          usedRawLabels.add(rawSpeaker);
        }
        localSpeakerAssignments.set(localSpeaker, {
          rawSpeaker,
          label: resolveSpeakerLabel(rawSpeaker, session, label),
        });
      } else {
        const rawSpeaker = getNextUnusedCanonicalRawLabel(usedRawLabels, seededSpeakerCount);
        localSpeakerAssignments.set(localSpeaker, {
          rawSpeaker,
...
function renderSpeakerFinalizeButton() {
  ...
  if (!hasSavedAudio) {
    elements.finalizeSpeakerButton.textContent = 'No saved audio yet';
    return;
  }
...
function getTranscriptDisplayCounts() {
  if (!state.currentSession && !state.transcriptVisibleRowCount) {
    return {
      visibleRows: 0,
      savedSegments: 0,
    };
  ...
  return {
    visibleRows: state.transcriptVisibleRowCount || 0,
    savedSegments: Number(state.currentSession?.segmentCount ?? getRenderableTranscriptSegments().length),
  };
}
...
function renderDrafts({ immediate = false } = {}) {
  ...
  scheduleTranscriptRender({ immediate });
}

function cancelScheduledTranscriptRender() {
  if (!state.transcriptRenderFrame) return;
  window.cancelAnimationFrame(state.transcriptRenderFrame);
  state.transcriptRenderFrame = 0;
}

function resetTranscriptRenderState() {
  cancelScheduledTranscriptRender();
  state.transcriptVisibleRowCount = 0;
  state.transcriptHiddenRowCount = 0;
}

function scheduleTranscriptRender({ immediate = false } = {}) {
  if (immediate) {
    cancelScheduledTranscriptRender();
    renderTranscript();
    return;
  ...
  if (state.transcriptRenderFrame) return;
  state.transcriptRenderFrame = window.requestAnimationFrame(() => {
    state.transcriptRenderFrame = 0;
    renderTranscript();
  });
}
...
function getTranscriptFeedSourceSegments() {
  const segments = getRenderableTranscriptSegments();
  if (state.currentSession?.status !== 'active' || segments.length <= LIVE_TRANSCRIPT_BUILD_SEGMENT_LIMIT) {
    return {
      segments,
  ...
  return {
    segments: segments.slice(-LIVE_TRANSCRIPT_BUILD_SEGMENT_LIMIT),
    trimmedSegmentCount: Math.max(0, segments.length - LIVE_TRANSCRIPT_BUILD_SEGMENT_LIMIT),
  };
}

function buildTranscriptFeedRows(sourceLabel, targetLabel) {
  const rows = [];
  const { segments, trimmedSegmentCount } = getTranscriptFeedSourceSegments();
...
function limitVisibleTranscriptRows(rows = [], liveRow = null) {
  const session = state.currentSession;
  const limit = session?.status === 'active' ? LIVE_TRANSCRIPT_MAX_VISIBLE_ROWS : 0;
  const liveCount = liveRow ? 1 : 0;
  const totalVisible = rows.length + liveCount;

  if (!limit || totalVisible <= limit) {
    return {
      rows,
      liveRow,
      hiddenCount: 0,
  ...
  const rowBudget = Math.max(0, limit - liveCount);
  const nextRows = rowBudget > 0 ? rows.slice(-rowBudget) : [];
  return {
    rows: nextRows,
    liveRow,
    hiddenCount: Math.max(0, totalVisible - (nextRows.length + liveCount)),
  };
}
...
function renderTranscriptHistory() {
  ...
  const { rows: feedRows, trimmedSegmentCount } = buildTranscriptFeedRows(sourceLabel, targetLabel);
  ...
  const { rows: displayRows, liveRow: displayLiveRow, hiddenCount } = limitVisibleTranscriptRows(mergedRows, appendedLiveRow);

  state.transcriptVisibleRowCount = displayRows.length + (displayLiveRow ? 1 : 0);
  state.transcriptHiddenRowCount = hiddenCount;

  if (!displayRows.length && !displayLiveRow) {
    ...
    const hiddenNote = hiddenCount || trimmedSegmentCount
      ? `<div class="note">Showing the latest ${state.transcriptVisibleRowCount} row${state.transcriptVisibleRowCount === 1 ? '' : 's'} during live capture.${hiddenCount ? ` ${hiddenCount} older row${hiddenCount === 1 ? '' : 's'} stay saved and reappear after you stop, end, or reopen the session from History.` : ' Older transcript stays saved and reappears after you stop, end, or reopen the session from History.'}</div>`
      : '';

    const rowMarkup = displayRows
      .map((row, index) => {
        const isNewestMergedRow = mergedIntoLastRow && index === displayRows.length - 1;
    ...
    list.innerHTML = `${hiddenNote}${rowMarkup}${displayLiveRow ? renderTranscriptFeedPair(displayLiveRow, { live: true, showPendingNote: false }) : ''}`;
  }
...
function updatePlaybackHighlight({ shouldScroll = false, forceScroll = false } = {}) {
  const audio = elements.reviewAudio;
  const playbackActive = Boolean(state.sessionPlaybackPreviewMs !== null || (audio && audio.src && !audio.paused && !audio.ended));
  if (!playbackActive && !state.sessionPlaybackSegmentId) return;
...
function renderManualSpeakerControls() {
  if (!elements.speakerChangeTimer) return;
  state.manualSpeakerUiLastRenderedAt = Date.now();
  const session = state.currentSession;
...
async function markRecordingsPendingDiarization(recordings = []) {
  if (!recordings.length) return;
  const updatedIds = new Set();
  ...
  for (const recording of recordings) {
    ...
    if (!id || updatedIds.has(id)) continue;
    updatedIds.add(id);
  ...
  if (updatedIds.size) {
    state.sessionRecordings = state.sessionRecordings.map((recording) => {
      const id = String(recording?.id || '').trim();
      if (!id || !updatedIds.has(id)) return recording;
      return {
        ...recording,
        diarizedAt: '',
      };
...
async function resetAutomaticSpeakerLabelsForSession(session = state.currentSession) {
  if (!session) return 0;
  ...
  for (const segment of sessionSegments) {
    ...
    const nextLabel = String(manualLabel || '').trim();
    const nextStatus = nextLabel ? 'done' : state.speakerTrackingSupported ? 'pending' : 'unsupported';
    const nextScore = nextLabel ? Math.max(Number(segment.speakerScore || 0), 1) : 0;
    const nextConfidence = nextLabel ? 1 : 0;

    if (
      !String(segment.speakerRawLabel || '').trim() &&
      String(segment.speakerLabel || '').trim() === nextLabel &&
      Number(segment.speakerScore || 0) === nextScore &&
      Number(segment.speakerConfidence || 0) === nextConfidence &&
      String(segment.speakerStatus || '').trim() === nextStatus
    ) {
      continue;
    }
    ...
    const updatedSegment = {
      ...
      speakerScore: nextScore,
      speakerConfidence: nextConfidence,
      speakerDurationMs: nextLabel ? Math.max(Number(segment.speakerDurationMs || 0), Number(segment.endMs || 0) - Number(segment.startMs || 0)) : 0,
      speakerStatus: nextStatus,
  ...
  if (updatedSegments.length && state.currentSession?.id === sessionId) {
...
function clearSpeakerFinalizeAutoTimer() {
  if (typeof window === 'undefined') return;
  window.clearTimeout(state.speakerFinalizeAutoTimer);
  state.speakerFinalizeAutoTimer = null;
}

function scheduleAutomaticSpeakerFinalize({ reason = 'stopped', delayMs = 600 } = {}) {
  if (typeof window === 'undefined') return;
  clearSpeakerFinalizeAutoTimer();
  if (!state.currentSession || !state.speakerTrackingSupported) return;
...
async function diarizeRecordingClip(recording, session = state.currentSession, { assignmentState = null } = {}) {
  ...
  const canonicalSegments = buildCanonicalFinalDiarizedSegments({
    diarizedSegments: diarized.segments.map((segment) => ({
      ...segment,
      startMs: Number(recording.startMs || 0) + Math.round(Number(segment.start || 0) * 1000),
      endMs: Number(recording.startMs || 0) + Math.round(Number(segment.end || 0) * 1000),
    })),
    session,
    assignmentState,
  });
  ...
  return applyDiarizedRecordingPass(
    {
      ...
      diarizedSegments: canonicalSegments.map((segment) => ({
        ...segment,
        start: Math.max(0, (Number(segment.startMs || 0) - Number(recording.startMs || 0)) / 1000),
        end: Math.max(0, (Number(segment.endMs || 0) - Number(recording.startMs || 0)) / 1000),
      })),
...
async function diarizeRecordingBatch(batch, session = state.currentSession, { assignmentState = null } = {}) {
...
async function diarizeRecordingClipWithRetry(
  recording,
  session = state.currentSession,
  { maxAttempts = SPEAKER_FINALIZE_MAX_ATTEMPTS, onRetry, assignmentState = null } = {}
) {
  let lastError = null;
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    try {
      const applied = await diarizeRecordingClip(recording, session, { assignmentState });
      return {
        ok: true,
        applied,
        attempts: attempt,
...
async function diarizeRecordingBatchIndividually(
  batch,
  session = state.currentSession,
  { maxAttempts = SPEAKER_FINALIZE_MAX_ATTEMPTS, onRetry, onRecordingProgress, assignmentState = null } = {}
) {
...
async function diarizeRecordingBatchWithRetry(
  batch,
  session = state.currentSession,
  { maxAttempts = SPEAKER_FINALIZE_MAX_ATTEMPTS, onRetry, assignmentState = null } = {}
) {
  if (batch?.recordings?.length === 1) {
    const result = await diarizeRecordingClipWithRetry(batch.recordings[0], session, { maxAttempts, onRetry, assignmentState });
    return {
      ...result,
      mode: 'single',
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    try {
      const applied = await diarizeRecordingBatch(batch, session, { assignmentState });
      return {
        ok: true,
        applied,
        attempts: attempt,
...
async function applyManualSpeakerEventsToCurrentSession({ unresolvedStatus = null } = {}) {
  if (!state.currentSession) return;
  const session = state.currentSession;
  ...
  for (const segment of state.currentSegments) {
    ...
    const nextConfidence = manualLabel
      ...
      : automaticLabel
        ...
        : 0;
    const nextStatus = nextLabel ? 'done' : unresolvedStatus || segment.speakerStatus;

    if (
      String(segment.speakerLabel || '').trim() === nextLabel &&
      (!manualLabel || Number(segment.speakerConfidence || 0) === nextConfidence) &&
      String(segment.speakerStatus || '').trim() === String(nextStatus || '').trim()
    ) {
      continue;
    }
...
async function stopListening() {
  ...
  renderCurrentView();
  scheduleAutomaticSpeakerFinalize({ reason: 'stopped' });
}
...
async function endCurrentSession() {
  ...
  renderCurrentView();
  scheduleAutomaticSpeakerFinalize({ reason: 'ended' });
...
async function finalizeSegmentFromEvent(event) {
  ...
  upsertCurrentSegmentInState(labeledSegment);
  await upsertSegment(labeledSegment);
  state.commitMetaByItemId.delete(itemId);
  clearDraftState(itemId, { preserveVisibleDraft: true });
  await persistCurrentSessionNow();
  renderCurrentView();
